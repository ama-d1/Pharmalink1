...
public class CommunityController {
    ...
    // NEW — the counterpart to /join above, which previously had none: once a
    // user joined a community there was no endpoint that could ever undo it.
    // Same ownership check as every other mutation here, so a caller can only
    // remove their own membership, not someone else's.
    @PostMapping("/{communityId}/leave")
    public ResponseEntity<?> leave(@PathVariable String communityId, @RequestBody Map<String, String> body, HttpServletRequest request) {
        if (!AuthContext.isOwnerOrAdmin(request, body.get("userId"))) return forbidden();
        try {
            communityService.leaveCommunity(communityId, body.get("userId"));
