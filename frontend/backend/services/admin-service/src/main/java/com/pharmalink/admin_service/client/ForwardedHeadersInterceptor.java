    ...
    public ClientHttpResponse intercept(HttpRequest request, byte[] body, ClientHttpRequestExecution execution) throws IOException {
        if (RequestContextHolder.getRequestAttributes() instanceof ServletRequestAttributes servletAttrs) {
            ...
            for (String header : FORWARDED_HEADERS) {
                ...
                // FIXED: containsKey() no longer exists — Spring Framework 7
                // (used by Spring Boot 4.x) stopped HttpHeaders from
                // implementing MultiValueMap, so the inherited Map methods
                // (containsKey/keySet/entrySet) are gone. containsHeader()
                // is the direct replacement, confirmed against the Spring
                // Framework 7.0.8 HttpHeaders javadoc.
