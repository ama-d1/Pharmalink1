...
// (MICROSERVICES_PLAN.md §5.2) — not solved here.
//
// The old monolith's SecurityConfig also declared a PasswordEncoder
// (BCryptPasswordEncoder) bean — that was only ever used by AuthService,
// which left the monolith back in step 1. Confirmed via grep that nothing
// else referenced it, so it's dropped here rather than carried over dead.
@Configuration
@EnableWebSecurity
