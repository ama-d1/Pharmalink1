package com.pharmalink.payment_service.config;
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
            ...
            .authorizeHttpRequests(auth -> auth
                // Ownership checks happen inside PaymentController itself
                // (AuthContext.isOwnerOrAdmin against the forwarded
                // X-User-Id header) — same Phase 2 posture as every other
                // service, permitAll here just means "reachable", not
                // "unchecked". /callback/** is genuinely unauthenticated on
                // purpose (a redirect from Paystack's own domain carries no
                // JWT) — see PaymentController's callback() javadoc.
